...
import { useSummaryRw } from "../../../../hooks/Questionnaire/useSubmission";
import { questionnaireService } from "../../../../service/Questionnaire/questionnaireService";
import { getKaderRwDashboardColumn, type KaderRTDataRow } from "../Columns/kaderRwDashboardColumn";
import MentalHealthChart from "../../../../components/MentalHealthChart";

const { RangePicker } = DatePicker;

export default function KaderResult() {
    const navigate = useNavigate();
    const { questionnaireId, rwId } = useParams<{ questionnaireId: string, rwId: string }>();
    const { user } = useAuth();
    ...
    const kaderRwId = user?.userDetail?.RukunWargaId;
    ...
    useEffect(() => {
        if (questionnaireId) {
            questionnaireService.getPublicQuestionnaireById(questionnaireId)
                .then(res => setQuestionnaireTitle(res?.title || "-"))
                .catch(() => setQuestionnaireTitle("Detail Kuesioner"));
        }
    }, [questionnaireId]);
    ...
    const columns = getKaderRwDashboardColumn({
        onSeeDetail: (rtId) => {
            navigate(`/summary-rt/${questionnaireId}/${rwId}/${rtId}`);
        },
    });
    ...
    if (!kaderRwId) {
        return (
            <div className="h-[70vh] flex items-center justify-center flex-col gap-4">
                <Empty description="Data Wilayah Kader Tidak Ditemukan" />
        ...
        <div className="flex flex-col w-full h-full min-h-screen bg-gray-50 pb-20">
            <div className="px-4 py-6 sm:px-6 lg:px-8">
                ...
                {summaryData && (
                    <div className="mb-6">
                        <MentalHealthChart
                            ...
                            title={`Statistik Kesehatan Mental - Wilayah RW Anda`}
                            subtitle={`Persentase Kondisi Mental Warga per RT`}
                        />
                    </div>
                )}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-y-5">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
                        <Breadcrumb
                            items={[
                                ...
                                {
                                    title: <span className="font-semibold text-gray-800">{questionnaireTitle}</span>,
                                },
                            ]}
                        />
                    ...
                    <Table
                        columns={columns}
                        dataSource={summaryData?.perRt as KaderRTDataRow[]}
                        rowKey="rtId"
                        loading={isFetching}
                        pagination={{
                            pageSize: 10,
